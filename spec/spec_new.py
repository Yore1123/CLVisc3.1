...
class Spec:
    ...
    def get_dNdphi(self, ylo=-0.5, yhi=0.5):
        ...
        fout_name = self.path + "/dNdPhi_{rapidity}".format( \
            rapidity = self.rapidity_kind) + self.strout

        np.savetxt(fout_name, np.array(list(zip(PHI, spec_along_phi))), \
            header='#dN/dNdphi_{rapidity}'.format( \
            rapidity=self.rapidity_kind))
