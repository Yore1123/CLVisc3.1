...
NucleusPtr Nucleus::create(const std::string& species, double nucleon_dmin) {
  ...
  else if (species == "Ru") // A, R0, a0, beta2, beta3,beta4,gamma
    return NucleusPtr{new DeformedWoodsSaxonNucleus2{
      96, 5.09, 0.460, 0.162,0.0,0.0,0.0    //2206.10449
    }};
  else if (species == "Ru2")
    return NucleusPtr{new DeformedWoodsSaxonNucleus2{
      96, 5.09, 0.460, 0.06,0.0,0.0,0.0
    }};
  else if (species == "Ru3")
    return NucleusPtr{new DeformedWoodsSaxonNucleus2{
      96, 5.09, 0.460, 0.06,0.2,0.0,0.0
    }};
  else if (species == "Ru4")
    return NucleusPtr{new DeformedWoodsSaxonNucleus2{
      96, 5.09, 0.52, 0.06,0.2,0.0,0.0
    }};
  else if (species == "Zr")
    return NucleusPtr{new DeformedWoodsSaxonNucleus2{
      96, 5.02, 0.52, 0.06,0.2,0.0,0.0
    }};
...
// "effective" radius.  The numerical coefficients for beta2 and beta4 are the
// approximate values of Y20 and Y40 at theta = 0.
DeformedWoodsSaxonNucleus2::DeformedWoodsSaxonNucleus2(
    std::size_t A, double R, double a, double beta2, double beta3, double beta4,double gamma)
    : Nucleus(A),
      R_(R),
      a_(a),
      beta2_(beta2),
      ...
      beta4_(beta4),
      gamma_(gamma),
      rmax_(R*(1. + .63*std::fabs(beta2) + std::fabs(beta3) + .85*std::fabs(beta4)) + 10.*a)
{}
...
/// this radius determines the impact parameter range, the true maximum radius
/// would cause far too many events with zero participants.
double DeformedWoodsSaxonNucleus2::radius() const {
  return rmax_ - 7.*a_;
}

double DeformedWoodsSaxonNucleus2::deformed_woods_saxon_dist(
    double r, double cos_theta, double phi) const {
  auto cos_theta_sq = cos_theta*cos_theta;
  auto sin_theta_sq = 1 - cos_theta*cos_theta;


  // spherical harmonics
  using math::double_constants::one_div_root_pi;
  auto Y20 = std::sqrt(5)/4. * one_div_root_pi * (3.*cos_theta_sq - 1.);
  auto Y22 = std::sqrt(15./2.)/4. * one_div_root_pi * sin_theta_sq*std::cos(2*phi);
  // where only pick up the real part of Y22
  auto Y30 = std::sqrt(7)/4. * one_div_root_pi * (-3.*cos_theta + 5.*cos_theta*cos_theta_sq);
  auto Y40 = 3./16. * one_div_root_pi *
             (35.*cos_theta_sq*cos_theta_sq - 30.*cos_theta_sq + 3.);
...
void DeformedWoodsSaxonNucleus2::sample_nucleons_impl() {
  // The deformed W-S distribution is defined so the symmetry axis is aligned
  // with the Z axis, so e.g. the long axis of uranium coincides with Z.
  //
  // After sampling positions, they must be randomly rotated.  In general this
  ...
  const auto sin_b = std::sin(angle_b);
  std::ofstream fout;
  //fout.open("pos_nucleus.dat",std::ios_base::app); //wxy_test
  for (auto&& nucleon : *this) {
    ...
    do {
      ...
      phi = random::phi<double>();

    } while (random::canonical<double>() > deformed_woods_saxon_dist(r, cos_theta,phi));
    ...
    auto y_rot = x*sin_b + y*cos_a*cos_b - z*sin_a*cos_b;
    auto z_rot = y*sin_a + z*cos_a;
    //fout << x <<" "<< y <<" "<<z<<std::endl; //wxy_test
    //auto x_rot = x; //wxy_test
    //auto y_rot = y; //wxy_test
    set_nucleon_position(nucleon, x_rot, y_rot, z_rot);
  }
  //fout.close();
}
