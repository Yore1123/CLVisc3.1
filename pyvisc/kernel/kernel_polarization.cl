        ...
        const int anti_particle)
{
    ...
    real density = 0.0f;
    real baryon_number = anti_particle > 0 ? -1.0f:1.0f;
...
#ifdef BARYON_ON
        mu = baryon_number*d_nbmutp[4*i + 1];
	if(mass < 1.116)
        { 
	    mu = baryon_number*d_nbmutp[4*i + 1]/3.0;
	}
#else
        mu = 0.0f; 
#endif
    ...
    if ( lid == 0 ) {
        ...
        d_pol_lab[gid] = pol_lab/(4.0f*mass)*2.0f;
        

        // polarization at (mt, Y, px, py) in lab frame
        real4 Pi_lab = pol_lab/(4.0f*mass)*2.0f;
...
#ifdef BARYON_ON
__kernel void polarization_chemical_on_sf(
        __global real4 * d_pol_lab,
        __global real * d_density,
        __global real4 * d_pol_lrf,
        __global const real8 * d_sf,
	...
	const int anti_particle)
{
	...
	if(mass < 1.116)
        ...
        // pomega^a
            ...
            subspec[lid] = volum * feq;
            subpol[lid] = hbarc*volum * pomega * feq * (1.0f - feq)/(pdotu);
            //subpol[lid] = hbarc* pomega * feq * (1.0f - feq)/(pdotu);
            //subpol[lid] = hbarc*volum *  feq * (1.0f - feq)/(pdotu);
            real pol0 = subpol[lid].s0;
            real pol3 = subpol[lid].s3;
            subpol[lid].s0 = cosh(etas)*pol0 + sinh(etas)*pol3;
            subpol[lid].s3 = sinh(etas)*pol0 + cosh(etas)*pol3;
            //subpol[lid].s3 =  cosh(etas)*pol3;
            //subpol[lid].s3 = sinh(etas)*pol0;
